...
#Frame size depends on rom - should be 2048 now, but leaving out in case of future issues

#OT_FRAME_SIZE_HACK = 1024
OT_FRAME_SIZE_HACK = 2048
...
class _Bootstrap:
  ...
  >>>   ...
  """
  _PIN_SCK='USB_SPI_SCK'
  _PIN_SDI='USB_SPI_COPI'
  _PIN_SDO='USB_SPI_CIPO'
  _PIN_CS='USB_SPI_CS'
  _PIN_TRST='USB_A17'
  _PIN_SRST='USB_A18'
  _PIN_JTAG_SPI='USB_A19'
  _PIN_BOOTSTRAP='USB_A16'
  # Delays below are in seconds.
  _BOOTSTRAP_DELAY=0.1
  _FIRST_FRAME_DELAY=0.2
  _INTER_FRAME_DELAY=0.02
...
def main(argv):
    ...
    bergparser = argparse.ArgumentParser(description='Load Bergen-Board Bitstream & Software')
   
    bergparser.add_argument('--bitstream', '-bs',
                           metavar='bitstream',
                           type=str,
                           help='Path to the FPGA .bit file to load',
                           default=None)

    bergparser.add_argument('--firmware', '-fw',
                           metavar='firmware',
                           type=str,
                           help='Path to the software .bin file to load',
                           default=None) 

    args = bergparser.parse_args()

    print("Bergen Loader: Attemping to find CW310 Bergen Board:")
    if args.bitstream:
    ...
    if args.firmware:
        ...
        prog = BergenSPIProgrammer(args.firmware)
        prog.run(target)
